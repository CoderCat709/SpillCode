...
# load a 32×32 vertical sprite sheet
...
#load a fullscreen sprite sheet (auto-detect tile size) 
...
#load a trigger/collection animation sheet
# disse er 50x50 per frame.
def load_sheet_trigger(path):
    sheet    = pygame.image.load(path).convert_alpha()
    img_w    = sheet.get_width()
    img_h    = sheet.get_height()
    tile     = 50
    cols     = max(1, img_w // tile)
    rows     = max(1, img_h // tile)
    frames   = []
    for row in range(rows):
        for col in range(cols):
            cell = sheet.subsurface((col * tile, row * tile, tile, tile))
            frames.append(pygame.transform.scale(cell, (WIDTH, HEIGHT)))
    return frames
...
#Load collection animations (bone pickup triggers)
# These play fullscreen when the player collects each bone type.
...
#Load menu/death backgrounds
try:
    bg_menu  = scale_bg("screen/start_menu.png")
except Exception:
    bg_menu  = None   # error fiks
 
try:
    bg_death = scale_bg("screen/Death_Screen.png")
except Exception:
    bg_death = None   # error fiks
 
# ── Spawn animation state ──────────────────────────────────────────────────────
spawn_frame      = 0
spawn_timer      = 0
spawn_phase      = "intro"   # intro, Spawn_animasjon, "end"  Spawn_ani_END
SPAWN_ANIM_SPEED = 6         # ticks / fps per frame
 
# ── Collection animation state ────────────────────────────────────────────────
# Plays on top of everything, blocks input briefly, then returns to "playing".
collect_anim_active = False   # True while a collection anim is playing
collect_anim_frames = []      # current frame list being played
collect_anim_frame  = 0
collect_anim_timer  = 0
COLLECT_ANIM_SPEED  = 6       # ticks per frame
...
#Platform definitions / hitboxes (pygame.Rect)
PLATFORMS_ROOM1 = [
...
while True:
    ...
    for event in pygame.event.get():
        ...
        # stopper hvis du ikke har collected beina i spillet
    ...
    # ══════════════════════════════════════════════════════════════════════════
    if game_state == "menu":
        # Draw background image if available, otherwise solid colour
        if bg_menu:
            screen.blit(bg_menu, (0, 0))
        else:
            screen.fill(BG_COLOR)
 
        # Dark overlay so buttons are always readable over any background
    ...
    # ── Movement, Prøver og fikse collection problem
    moving = False
    if not collect_anim_active:
    ...
    if not collect_anim_active:
        ...
        if px + CHAR_W <= 0:
            high_dest   = room.get("middel_high")
            high_thresh = room.get("middel_high_threshold", 999)
            if high_dest and py < high_thresh:
                current_room = high_dest; reset_player("right")
            elif room["left"]:
                current_room = room["left"]; reset_player("right")
    ...
    # ── collection animation overlay, veldig buggy
    # runner over hele skjermen, må legge til en bakkgrunn for å unngå rare artefakter når den spiller
    if collect_anim_active:
        ...
        if collect_anim_timer >= COLLECT_ANIM_SPEED:
            ...
            if collect_anim_frame >= len(collect_anim_frames):
                # Animation ferdig — return to normal gameplay
